...
- **Per-Bot Linux computers (v2.0)** — each Bot gets its own QEMU/KVM VM on a Linux server you control (libvirt, virbr0, cloud-init, Ubuntu 24.04 + XFCE). MaxBot drives libvirt over SSH, generates per-Bot Ed25519 keypairs (encrypted at rest with Argon2id + ChaCha20-Poly1305), and exposes a 300ms host-side screenshot preview plus an in-panel click-through **Drive** (v3.7.9+) that forwards pointer + keyboard to xdotool over the same SSH connection. No SaaS, no remote-desktop provider, no in-webview VNC canvas, no external viewer.
...
- **Three access levels per Bot (Status / Preview / Drive)** — a tiny status chip in the title bar; a pinned Preview side panel with a fresh JPEG from the QEMU framebuffer; an in-panel **Drive** (v3.7.9+) that lets you click and type on the JPEG to take over for 2FA or step in. The human can see what the Bot is doing at any time, with full keyboard + mouse control during Drive. Multi-VM installs allocate VNC ports sequentially (5900, 5901, 5902, …) — see `docs/user-guide.md#computer` for the v3.7.2+ flow.
