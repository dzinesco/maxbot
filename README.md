...
- **Per-Bot Linux computers (v2.0)** — each Bot gets its own QEMU/KVM VM on a Linux server you control (libvirt, virbr0, cloud-init, Ubuntu 24.04 + XFCE). MaxBot drives libvirt over SSH, generates per-Bot Ed25519 keypairs (encrypted at rest with Argon2id + ChaCha20-Poly1305), and exposes a 300ms host-side screenshot preview plus a "Take over with Screen Sharing" macOS-native takeover tunnel (v3.7.2). No SaaS, no remote-desktop provider, no in-webview VNC canvas.
...
- **Three access levels per Bot (Status / Preview / Takeover)** — a tiny status chip in the title bar; a pinned Preview side panel with a fresh JPEG from the QEMU framebuffer; full-window Takeover that hands the user a `vnc://127.0.0.1:<port>` link for macOS Screen Sharing. The human can see what the Bot is doing at any time, with full keyboard + mouse control during Takeover. See `docs/user-guide.md#computer` for the v3.7.2+ flow.
