...
pub struct State {
    ...
    /// Last turn that completed successfully (journal entry written).
    /// Invariant: `completed_turn <= turn`. When `completed_turn == turn`
    /// no turn is in progress; when `completed_turn == turn - 1` turn
    /// `turn` was started but not finished (a previous supervisor
    /// process was killed mid-turn — the boot path detects this and
    /// completes or skips the in-progress turn).
